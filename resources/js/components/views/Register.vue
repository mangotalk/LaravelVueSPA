<template>
    <div class="w-full p-4 mt-1 md:mt-8">
        <div class="bg-white max-w-md border border-grey-light rounded mx-auto">
            <div class="py-3 px-4 border-b border-grey-light">Register</div>
            <form class="py-6" v-on:submit.prevent="register">
                ...
                <div class="mx-auto w-3/4 flex flex-wrap flex items-stretch h-full py-4">
                    <div class="w-1/4 text-right pr-2 self-start" :class="{ 'text-red-dark': form.errors.password_confirmation }">
                        Confirm Password
                    </div>
                    <div class="w-3/4 text-left px-4 self-start">
                        <input type="password" v-model="form.password_confirmation" :class="{ 'border-red-dark': form.errors.password_confirmation }" class="bg-white rounded border w-full px-2 py-2 focus:outline-none text-grey-darker"/>
                        <p v-if="form.errors.password_confirmation" class="text-red-dark mt-1 text-sm">{{ form.errors.password_confirmation }}</p>
                    </div> 
                </div>
                <div class="mx-auto w-3/4 flex flex-wrap flex items-stretch h-full py-3">
                    <div class="w-1/4 text-right px-2 self-start pt-2"></div>
                    <div class="w-3/4 text-left px-4 self-start">
                        <button class="px-4 py-2 focus:outline-none text-white bg-blue-dark rounded">Register</button>
                    </div> 
                </div>
            </form>
        </div>
...
export default {
    ...
    methods: {
        register() {
            ...
            axios.post('/api/auth/register', _this.form)
                .then((response) => {
